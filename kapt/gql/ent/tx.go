...
func (tx *Tx) init() {
	tx.AttachmentModel = NewAttachmentModelClient(tx.config)
	tx.CategoryModel = NewCategoryModelClient(tx.config)
	tx.TenantModel = NewTenantModelClient(tx.config)
	tx.ThreadModel = NewThreadModelClient(tx.config)
	tx.TicketModel = NewTicketModelClient(tx.config)
...
// applies a query, for example: AttachmentModel.QueryXXX(), the query will be executed
